# train_eval_bert.py (optimizado para RTX 6000 Ada)
import os, random
...
SEED = 42
EPOCHS = 2
BATCH_SIZE = 32                 # RTX 6000 Ada puede manejarlo fácilmente
LEARNING_RATE = 3e-5
PATIENCE = 2
MAX_LEN = 128                   # 256 si las frases son muy largas
MODEL_NAME = "bert-large-uncased"  # o 'bert-base-uncased' si prefieres menor tamaño
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if acc > best_val_acc:
            ...
            best_model_path = f"models/bert-finetuned-epoch{epoch+1}-acc{acc:.4f}"
            model.save_pretrained(best_model_path); tokenizer.save_pretrained(best_model_path)
            print(f"Mejor modelo guardado en: {best_model_path}")
            ts = datetime.now().strftime("%Y%m%d-%H%M%S")
            plot_confusion_matrix(yt, vp, label_encoder.classes_, title=f"Matriz de Validación (Best Epoch {epoch+1}) - BERT", outfile=f"matriz_confusion_valid_best_bert_{ts}.png")
    ...
    plt.title("Curvas de Validación (BERT)"); plt.xlabel("Época"); plt.ylabel("Score"); plt.grid(); plt.legend()
    ...
    plot_confusion_matrix(yt, yp, label_encoder.classes_, title="Matriz de Confusión (Test) - BERT", outfile=png)
