#!/usr/bin/env python3
# train_eval_bert_3050.py
# BERT (base) afinado para 6GB VRAM con AMP + grad. accumulation + checkpointing
...
def main():
    ...
    model.gradient_checkpointing_enable()  # ahorro extra memoria

    optimizer = AdamW(model.parameters(), lr=LEARNING_RATE)
    # Scheduler por actualización (con acumulación)
    ...
    for epoch in range(EPOCHS):
        ...
        if acc > best_val_acc:
            ...
            best_model_path = f"./bert-base-finetuned-epoch{epoch+1}-acc{acc:.4f}"
            ...
            plot_confusion_matrix(yt, vp, label_encoder.classes_,
                                  title=f"Matriz de Validación (Best Epoch {epoch+1}) - BERT-base",
    ...
    plt.title("Curva de Pérdida (BERT-base)"); plt.xlabel("Época"); plt.ylabel("Loss"); plt.grid(); plt.legend()
    ...
    plt.title("Curvas de Validación (BERT-base)")
    ...
    plot_confusion_matrix(yt, yp, label_encoder.classes_,
                          title="Matriz de Confusión (Test) - BERT-base", outfile=png)
