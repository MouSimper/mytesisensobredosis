#!/usr/bin/env python3
# train_eval_roberta.py
# Fine-tuning de RoBERTa para clasificación de texto
# Autor: Adaptado para columnas English_clean y Classification_English
...
def main():
    ...
    if not {"English_clean", "Classification_English"}.issubset(df.columns):
        raise ValueError("❌ El dataset debe contener las columnas 'English_clean' y 'Classification_English'")
    ...
    for epoch in range(EPOCHS):
        ...
        if acc > best_val_acc:
            ...
            plot_confusion_matrix(
                yt, vp, label_encoder.classes_,
                title=f"Matriz de Validación (Best Epoch {epoch+1}) - RoBERTa",
    ...
    plt.subplot(1, 2, 1)
    plt.plot(ep, tr_losses, marker='o', label="Train Loss")
    plt.title("Curva de Pérdida (RoBERTa)")
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.grid()
    plt.legend()
    ...
    plt.title("Curvas de Validación (RoBERTa)")
    plt.xlabel("Época")
    plt.ylabel("Score")
    plt.grid()
    plt.legend()
    plt.tight_layout()
    plt.savefig("curvas_entrenamiento_roberta.png", dpi=200)
    plt.show()
    ...
    plot_confusion_matrix(
        yt, yp, label_encoder.classes_,
        title="Matriz de Confusión (Test) - RoBERTa",
        outfile=png
    )
    ...
    try:
        dfm.to_excel(f"metricas_por_clase_test_roberta_{ts}.xlsx", index=True)
    except Exception:
        pass
