...
def plot_confusion_matrix(y_true, y_pred, class_names, title, outfile, max_len=6):
    ...
    # --- figura (deja margen derecho para leyenda) ---
    ...
    # Leyenda: abreviatura = nombre completo
    legend_text = "\n".join(f"{s} = {full}" for s, full in zip(short_labels, class_names))
    plt.tight_layout(rect=[0.0, 0.0, 0.80, 1.0])  # deja ~20% a la derecha
...
def main():
    ...
    plt.title("Curva de Pérdida (RoBERTa)"); plt.xlabel("Época"); plt.ylabel("Loss"); plt.grid(); plt.legend()
    ...
    plot_confusion_matrix(yt, yp, label_encoder.classes_,
                          title="Matriz de Confusión (Test) - RoBERTa", outfile=png)
